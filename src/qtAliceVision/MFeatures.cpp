...
void FeaturesIORunnable::run()
{
    ...
    std::unique_ptr<MFeatures::MViewFeaturesPerViewPerDesc> viewFeaturesPerViewPerDescPtr(
        new MFeatures::MViewFeaturesPerViewPerDesc);
    ...
    bool loaded = sfm::loadFeaturesPerDescPerView(regionsPerViewPerDesc, viewIds, {folder.toLocalFile().toStdString()},
                                                  imageDescriberTypes);
    ...
    for (int dIdx = 0; dIdx < descTypes.size(); ++dIdx)
    {
        ...
        for (std::size_t vIdx = 0; vIdx < viewIds.size(); ++vIdx)
        {
            ...
            MFeatures::MViewFeatures& viewFeatures =
                (*viewFeaturesPerViewPerDescPtr)[descTypes.at(dIdx).toString()][viewIds.at(vIdx)];
...
void MFeatures::loadGlobalInfo()
{
    ...
    FeaturesIORunnable* ioRunnable =
        new FeaturesIORunnable(FeaturesIORunnable::IOParams(_featureFolder, viewIds, _describerTypes));
...
void MFeatures::onFeaturesReady(MViewFeaturesPerViewPerDesc* viewFeaturesPerViewPerDesc)
{
    ...
    if (newFeaturesLoaded)
    {
        for (auto& viewFeaturesPerViewPerDescPair : *viewFeaturesPerViewPerDesc)
        {
            ...
            _viewFeaturesPerViewPerDesc[describerType].insert(viewFeaturesPerViewPerDescPair.second.begin(),
                                                              viewFeaturesPerViewPerDescPair.second.end());
        }
    }
...
void MFeatures::getViewIdsToLoad(std::vector<aliceVision::IndexT>& viewIdsToLoad)
{
            ...
            if (_timeWindow < 0) // no limit
            {
                for (const auto& viewPair : sfmData.getViews())
                {
                    ...
                    if ((currentIntrinsicId == view.getIntrinsicId() &&
                         (view.getFrameId() != aliceVision::UndefinedIndexT)))
                        viewIdsToLoad.emplace_back(view.getViewId());
                }
            }
            else // time window
            {
                const aliceVision::IndexT minFrameId = (currentFrameId < static_cast<uint>(_timeWindow))
                                                           ? 0
                                                           : currentFrameId - static_cast<uint>(_timeWindow);
                const aliceVision::IndexT maxFrameId = currentFrameId + static_cast<uint>(_timeWindow);

                for (const auto& viewPair : sfmData.getViews())
                {
                    const aliceVision::sfmData::View& view = *(viewPair.second);
                    if ((currentIntrinsicId == view.getIntrinsicId()) && (view.getFrameId() >= minFrameId) &&
                        (view.getFrameId() <= maxFrameId))
                        viewIdsToLoad.emplace_back(view.getViewId());
                }
            }
        }
    ...
    qDebug() << "[QtAliceVision] Features: Caching: " << viewIdsToKeep.size() << " frame(s) kept, "
             << viewIdsToRemove.size() << " frame(s) removed, " << viewIdsToLoad.size() << " frame(s) requested.";
}
...
bool MFeatures::updateFromTracks(MViewFeaturesPerViewPerDesc* viewFeaturesPerViewPerDesc)
{
    ...
    {
        qDebug() << "[QtAliceVision] Features: Unable to update from tracks, Tracks is not ready: "
                 << _mtracks->status();
        return updated;
    }
    ...
    for (auto& viewFeaturesPerViewPerDescPair : *viewFeaturesPerViewPerDesc)
    {
        const aliceVision::feature::EImageDescriberType descType =
            aliceVision::feature::EImageDescriberType_stringToEnum(viewFeaturesPerViewPerDescPair.first.toStdString());
        ...
        for (auto& viewFeaturesPerViewPair : viewFeaturesPerViewPerDescPair.second)
        {
            ...
            {
                qInfo() << "[QtAliceVision] Features: Update from tracks, view: " << viewId
                        << " does not exist in tracks";
                return updated;
            }
            ...
            for (const auto& trackId : tracksInCurrentView)
            {
                ...
                {
                    qInfo() << "[QtAliceVision] Features: Update from tracks, featId invalid regarding loaded features "
                               "for view: "
                            << viewId;
                    continue;
                }
...
bool MFeatures::updateFromSfM(MViewFeaturesPerViewPerDesc* viewFeaturesPerViewPerDesc)
{
    ...
    {
        qDebug() << "[QtAliceVision] Features: Unable to update from SfM, SfMData is not ready: "
                 << _msfmData->status();
        return updated;
    }
    ...
    for (auto& viewFeaturesPerViewPerDescPair : *viewFeaturesPerViewPerDesc)
    {
        ...
        for (auto& viewFeaturesPerViewPair : viewFeaturesPerViewPerDescPair.second)
        {
            ...
            {
                qInfo()
                    << "[QtAliceVision] Features: Update from SfM, SfMData has no valid pose and intrinsic for view: "
                    << viewId;
                continue;
            }
            ...
            const aliceVision::camera::IntrinsicBase* intrinsic =
                _msfmData->rawData().getIntrinsicPtr(view.getIntrinsicId());
            ...
            for (const auto& landmark : _msfmData->rawData().getLandmarks())
            {
                ...
                if (itObs != landmark.second.observations.end())
                {
                    ...
                    {
                        viewFeatures.features.at(static_cast<int>(itObs->second.id_feat))
                            ->setLandmarkInfo(static_cast<int>(landmark.first), r.cast<float>());
                    }
                    else if (!viewFeatures.features.empty())
                    {
                        qWarning() << "[QtAliceVision] [ERROR] Features: Update from SfM, view: " << viewId
                                   << ", id_feat: " << itObs->second.id_feat
                                   << ", size: " << viewFeatures.features.size();
                    }
...
void MFeatures::updatePerTrackInformation()
{
    ...
    for (const auto& landmark : _msfmData->rawData().getLandmarks())
    {
        const auto& describerTypeName =
            QString::fromStdString(aliceVision::feature::EImageDescriberType_enumToString(landmark.second.descType));
        ...
        for (auto& featuresPerFramePair : trackFeatures.featuresPerFrame)
        {
            ...
            if (!_msfmData->rawData().isPoseAndIntrinsicDefined(&view))
            {
                ...
                featuresPerFramePair.second->setReprojection(
                    {featuresPerFramePair.second->x(), featuresPerFramePair.second->y()});
                continue;
            }
...
void MFeatures::updateUIFeaturesInformation()
{
    ...
    for (auto& viewFeaturesPerViewPerDescPair : _viewFeaturesPerViewPerDesc)
    {
        ...
        for (auto& featuresPerViewPair : viewFeaturesPerViewPerDescPair.second)
        {
            ...
            featuresViewInfo.insert("frameId", (viewFeatures.frameId == aliceVision::UndefinedIndexT)
                                                   ? QString("-")
                                                   : QString::number(viewFeatures.frameId));
            featuresViewInfo.insert("nbFeatures", QString::number(viewFeatures.features.size()));
            featuresViewInfo.insert("nbTracks", (viewFeatures.nbTracks < 0) ? QString("-")
                                                                            : QString::number(viewFeatures.nbTracks));
            featuresViewInfo.insert("nbLandmarks", (viewFeatures.nbLandmarks < 0)
                                                       ? QString("-")
                                                       : QString::number(viewFeatures.nbLandmarks));
...
void MFeatures::clearAll()
{
    ...
    _trackFeaturesPerTrackPerDesc
        .clear(); // no need to delete feature pointers, they are hosted and manage by viewFeaturesPerViewPerDesc
